 ...
 *
 * The one interaction worth designing carefully is the copy-once panel. A key
 * exists in readable form for exactly one render, so it is shown with an
 * explicit warning, a copy control, and no way to dismiss it by accident — and
 * the list below never shows more than the prefix, because that is all the
 * server has.
 */
  ...
  return (
    <div className="flex flex-col gap-10">
      ...
      <form onSubmit={create} className="flex flex-col gap-5">
        ...
        <Field id="key-name" label="Name" hint="What this key is for — “CI pipeline”, “Zapier”.">
          {(aria) => (
            <Input
              {...aria}
              required
          ...
          <ul className="flex flex-col divide-y divide-border-subtle">
            {keys.map((key) => (
              <li key={key.id} className="flex flex-wrap items-center gap-x-6 gap-y-2 py-4">
                ...
                {key.revokedAt ? null : (
                  <Button type="button" variant="danger" size="sm" onClick={() => void revoke(key)}>
                    Revoke
                  </Button>
                )}
              </li>
