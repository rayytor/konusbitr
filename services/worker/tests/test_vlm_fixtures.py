...
from konusbitr_worker.parse import _apply_section_paths, _ordered
...
class TestReadingOrder:
    ...
    async def test_a_heading_scopes_the_elements_below_it(self, two_column) -> None:
        ...
        """
        [result] = await read(two_column, scripted_answer(two_column, corrupted=False))
        # `_ordered` interleaves one batch's tiers without renumbering; the
        # accumulator assigns ids across the whole document, and the section
        # paths are rebuilt over everything accumulated so far. Neither matters
        # to what this asserts, which is the trail itself.
        merged = _ordered(DoclingParse(markdown="", contents=[]), [], [result])
    ...
    async def test_the_vlm_reading_supersedes_the_other_tiers(self, two_column) -> None:
        ...
        merged = _ordered(docling, [], [result])
